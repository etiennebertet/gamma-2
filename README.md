# gamma-2
Automation of AI-generated images production, corresponding metadata files, and upload to hosting servers. 
...
The second class generates a text that will be exported as a .csv file when calling to the AWS Lambda Python script.
...
The sixth class upscales the image to be compliant with the hosting websites' requirements.

The seventh class calls a Python function hosted on AWS Lambda 

The AWS Lambda file 1.Hosts the upscaled image on S3 Bucket ; 2.Exports the CSV text into a .csv file on S3 Bucket ; 3.Launches an FTP transfer to a specialized hosting server (Shutterstock, Freepik, Adibe Cloud, etc.)
